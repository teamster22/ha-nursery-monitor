# Tablet build — original working notes

> Build journal, not instructions. See
> [../tablet-build.md](../tablet-build.md) for the guide you actually want.

This is the raw build proposal, lightly scrubbed, written to myself before and
during the build. It uses task IDs (N1, N2...) from my own kanban board and
section cross-references that only resolve inside this document. Kept because the
reasoning behind most decisions in the real guide lives here.
...
**Target behavior:** replicate the **Eufy SpaceView E10** parent unit (§3).
**Rev 5** — **N1 locked to Device Owner provisioning with no Google account (§8).** Power/volume button behavior settled (§3.8), and one earlier overclaim about the power menu corrected there.
...
| | |
|---|---|
| Behavior model | **Eufy E10 parity — dormant by default, wakes on qualified cry (§3)** |
...
## 3. The behavior model — Eufy E10 parity
...
- **Audio ceasing while dormant is the desired behavior**, so screen-off's audio-suspension is no longer a bug to work around.
...
### 3.6 Latency budget — **MEASURED, not estimated**
...
### 3.9 Measured detector behavior, from two live tests

Both tests played recorded crying in an empty nursery at crib distance. **This was the first time `binary_sensor.nursery_crying_sound` had ever fired** — before 13:36 on test day its entire history was `unavailable` → `off`.
...
**Consequence for the offline alert (§9.2):** it will fire when you let the battery die on the couch. That's correct behavior, not a bug — the low-battery warning at 20 % is what makes it predictable rather than annoying.
...
Android routes the volume keys to the **media stream** whenever media is playing. Because our design keeps the video element playing continuously (§3.1), media is *always* playing — so the volume keys always adjust nursery audio. Exactly the E10 behavior, for free.
...
| **WallPanel** (free, open source) | Viable but weaker exactly where it matters: no first-party HA integration (hand-rolled MQTT), and inconsistent autoplay/audio behavior across Android versions. |
...
- **The PLUS license can be purchased without a Google account** — Fully documents an instant-license path explicitly for this case. Buy from their site, not via Play billing.
...
5. **Buy and apply the PLUS license**, then enable **Remote Admin** with a password.

   ⚠️ **Enabling Remote Admin does NOT start the service — Fully must be restarted.** Observed on this build: the toggle was set and the password entered, yet nothing on the LAN answered on port 2323, and Fully Cloud's own remote view reported `Remote Admin service running: false` while successfully relaying to the device (so: network fine, app running, service simply not started). **Fix: Fully menu → Restart App.** Port 2323 came up immediately after.

   **Two diagnostics worth reusing:** an unlicensed Fully shows a **watermark** on screen — that's the fast way to tell a license problem from a service-start problem, since both present as Remote Admin being unreachable. And ping jitter is a reliable sleep indicator: the tablet read 5–46 ms with 16 ms mdev while dozing versus **2.9–7.3 ms with 1.5 ms mdev** awake.
...
8. **Network:** 5 GHz, static DHCP reservation on the Deco (by MAC, so it follows the tablet room to room).
   ...
   **✅ ASSIGNED — `<TABLET_IP>`, MAC `AA:BB:CC:DD:EE:FF`, reserved on the Deco.**
...
| Phase | Work | Est. |
|---|---|---|
| **N1** | ✅ **COMPLETE.** OS update → factory reset → Device Owner provisioning (no Google account) → PLUS license → Fully config → HA integration (33 entities). Final three settings closed out: **Start URL** (verified live — `sensor...current_page` reads `http://homeassistant.local:8123/nursery-monitor/monitor`, `foreground_app` = `com.fullykiosk.emm`), **Android Display size**, **Battery → Unrestricted**. ⚠️ **Still OFF and deliberate-or-not-yet-decided:** `binary_sensor...kiosk_mode`, `switch...kiosk_lock`, `binary_sensor...device_admin`. | done |
...
| **N6** | ✅ **COMPLETE.** 2 scripts + 6 automations live, area `Floating Devices`, labels `kiosk`+`baby`, category *Nursery Monitor*, icons set. `HA_Reference.md` reconciled 2,163 → 2,171, Repairs clean. **Verified by template evaluation only — nothing has been RUN.** See §7.36 for the six design decisions and the outstanding live tests. | done |
...
**Set at my request, for the duration of the build-out only.** Both baby-monitor alert automations were narrowed to my phone so setup testing doesn't repeatedly blast my wife with critical alerts:
